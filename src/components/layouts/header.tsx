...
const Header = () => {
    ...
    return (
        <>
            ...
            <div
                ref={headerRef}
                className={`w-full bg-[#fff] z-50 transition-transform duration-300 ease-in-out ${
            ...
            >
                    ...
                    <div className="w-full max-w-[2400px]">
                        <header className="flex h-[60px] items-center w-full gap-x-10">
                            <div className="flex items-center gap-[7px] flex-shrink-0">
                                <Link href="/">
                                    <Image alt="Wishzy logo" src="/logo/logo_black.png" width={100} height={45} />
                                </Link>
                            </div>
                                            ...
                                            {visibleItems.map((item) => (
                                                <div key={item.id} className="relative h-full flex items-center group mt-2">
                                                    <Button variant="ghost" className={`h-full gap-[5px] px-2 rounded-none hover:bg-transparent whitespace-nowrap border-b-2 transition-colors duration-300 ${activeDropdown === item.id ? "border-[#ffa500]" : "border-transparent"}`} onClick={() => handleNavClick(item.id)}>
                                                        <span className="font-normal text-[#000] text-sm tracking-[0] leading-[21px] transition-colors group-hover:text-[#000]">{item.label}</span>
                                                        <ChevronDown className={`w-3 h-3 text-[#000] transition-transform duration-300 group-hover:text-[#000] ${activeDropdown === item.id ? "rotate-180" : "rotate-0"}`} />
                                                    </Button>
                                                </div>
                                            ))}
                                            {hiddenItems.length > 0 && (
                                                <div className="relative h-full flex items-center group">
                                                    <Button variant="ghost" className={`h-full gap-[5px] px-2 rounded-none hover:bg-transparent border-b-2 transition-colors duration-300 mt-2 ${activeDropdown === "more" ? "border-[#ffa500]" : "border-transparent"}`} onClick={() => handleNavClick("more")}>
                                                        <span className="font-normal text-[#000] text-sm tracking-[0] leading-[21px] transition-colors group-hover:text-[#000] ">Khác</span>
                                                        <ChevronDown className={`w-3 h-3 text-[#000] transition-transform duration-300 group-hover:text-[#000] ${activeDropdown === "more" ? "rotate-180" : "rotate-0"}`} />
                                                    </Button>
                                                </div>
                                            )}
                                        </nav>
                                ...
                                <div className="hidden xl:flex items-center gap-5">
                                    {!isScrolled && (
                                        <Button variant="ghost" className="h-10 px-0 rounded-[15px] hover:bg-transparent hover:text-white transition-colors">
                                            <span className="font-normal text-[#000] text-sm leading-[21px]">Chính sách</span>
                                        </Button>
                                    )}
                                    ...
                                    <Button  className="w-[200px] h-10 px-[11px] py-0 bg-[#FFF5ED] rounded-[5px] font-medium text-black text-base leading-6 transition-colors">
                                        Khám phá khóa học
                                    </Button>
                                </div>
                ...
                {!isScrolled && (
                    <div className={`absolute top-[60px] left-0 w-full bg-[#fff] rounded-b-2xl border-t border-[#ccc] transition-all duration-200 ease-in-out hidden xl:block ${
